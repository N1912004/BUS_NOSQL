...
<head>
    ...
    <title>Thêm người soát vé </title>
    ...
    <div class="maindiv" id="maindiv" style="width: 25%;">
        ...
        <form action="AddCon.php" method="post">
            <div class="title">
                ...
                <h2 class="text-center">Thêm thông tin xác thực của người soát vé</h2>
                <br>
            </div>
            ...
            <div class="form-group">
                <div class="form-group">
                    <label>ID: </label>
                    <input class="form-control" type="text" placeholder="ID người soát" name="Cid" maxlength="4">
                </div>
                <div class="form-group">
                ...
                <br>
                <button class="btn-item btn-block" style="width: 100%;" name='sub' type="submit">Xác nhận</button>
            </div>
...
if (isset($_POST['sub'])) {
    ...
    $loginDocument = new ArangoDocument();
    $loginDocument->set('_key', $cid); // Using cid as _key
    $loginDocument->set('user_name', $cid);
    $loginDocument->set('password', $pwd);

    try {
        $documentHandler->save('login', $loginDocument);
        if ($pwd != '') {
            echo "<script>alert('Đã thêm thành công người soát vé!')</script>";
        }
    } catch (ArangoDBClient\Exception $e) {
        echo "<script>alert('Lỗi khi thêm người soát vé: " . $e->getMessage() . "')</script>";
    }
}
?>
