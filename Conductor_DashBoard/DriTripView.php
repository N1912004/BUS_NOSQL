...
$columns = ['trip_no', 'bus_no', 'Source', 'Destination', 'TripDate', 'Conductor_emp_id', 'scheduled_dept_time', 'scheduled_arr_time'];
...
function fetch_data($connection, $columns)
{
    ...
    if (empty($connection)) {
        ...
        $query = 'FOR b IN busDetails FILTER b.trip_no IN (FOR t IN tripIncharge FILTER t.Driver_emp_id == @DriID RETURN t.trip_no_incharge) LET ti = (FOR t_incharge IN tripIncharge FILTER t_incharge.trip_no_incharge == b.trip_no RETURN t_incharge) RETURN { trip_no: b.trip_no, bus_no: b.bus_no, Source: b.Source, Destination: b.Destination, TripDate: b.TripDate, Conductor_emp_id: ti[0].Conductor_emp_id, scheduled_dept_time: ti[0].scheduled_dept_time, scheduled_arr_time: ti[0].scheduled_arr_time }';
        
        $statement = new ArangoStatement($connection, [
            'query' => $query,
            'bindVars' => ['DriID' => $DriID]
        ]);
