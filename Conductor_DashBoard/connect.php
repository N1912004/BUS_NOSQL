...
$query = 'FOR p IN passenger SORT p.ticket_id DESC LIMIT 1 RETURN p.ticket_id';
$statement = new ArangoStatement($connection, ['query' => $query, '_flat' => true]);
$cursor = $statement->execute();
$lastTicketId = $cursor->current();

$TicketID = ($lastTicketId !== null) ? $lastTicketId + 1 : 1;
...
$passengerDocument = new ArangoDocument();
$passengerDocument->set('trip_no_passenger', (int)$TripNumber);
$passengerDocument->set('phone_no', $PhoneNumber);
$passengerDocument->set('ticket_id', (int)$TicketID);
$passengerDocument->set('Passenger_source', $sourceChoice);
$passengerDocument->set('Passenger_destination', $destinationChoice);
$passengerDocument->set('ticket_price', (int)$Ticketprice);

try {
    $documentHandler->save('passenger', $passengerDocument);
...
<head>
    ...
    <style>
        body {
            background: url("../Images/bus_ansuong.jpg") no-repeat center;
        ...
        <form action="connect.php" method="post">
            ...
            <div class="info">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control" type="number" id="tel" name="PhoneNumber" placeholder="Ví dụ: 0936535534"
                        required>
                </div>

                <div class="form-group">
                    <label>Chọn Nguồn Trạm xe buýt: </label>
                    <input class="form-control" type="text" name="sourceChoice" placeholder="Ví dụ: Bến xe Miền Đông" required>
